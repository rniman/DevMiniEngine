...
namespace Math
{
	...
	)
	{
		...
		// 출력 배열 초기화
		outTangents.clear();
		outTangents.resize(vertexCount, Vector3::Zero());
		...
		for (size_t i = 0; i < vertexCount; ++i)
		{
			...
			// 길이 체크
			Core::float32 length = tangent.Length();
			if (length > 1e-6f)
			{
				tangent.Normalize();
			}
			else
			{
				...
				if (std::abs(n.x) < 0.9f)
				{
					tangent = Vector3::UnitX().Cross(n).Normalized();
				}
				else
				{
					tangent = Vector3::UnitY().Cross(n).Normalized();
				}
			}
